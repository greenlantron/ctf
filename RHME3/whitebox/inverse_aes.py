...
def main():
    if len(sys.argv) < 2:
        print "Usage: %s <hex last round key>" % sys.argv[0]
        return
    
    try:
        cipher = custom_AES(sys.argv[1].decode("hex"))
    ...
    print "Inverse expanded keys = ["
    for key in invexpkey:
        print "\t%s" % key
    print "]"
    
    cipher = invexpkey[-1]
    print "Cipher key: %s" % cipher
    print "As string: %s" % repr(cipher.decode("hex"))
